...
def dino_model(student_model: torch.nn.Module, teacher_model: torch.nn.Module) -> Dino:
    ...
    return Dino(student=student_model, teacher=teacher_model, loss_function=torch.nn.MSELoss())
...
def test_update_teacher_weights(dino_model: Dino):
    ...
    assert torch.allclose(dino_model.teacher.weight, expected_weight), "Teacher weights were not updated correctly."
...
def test_train(dino_model: Dino):
    ...
    # When
    dino_model.train(lr=0.5, data_loader = fake_dataloader, epoch_number=1, optimizer_function = torch.optim.SGD)

    # Then

    # Student weights should have changed
    assert not torch.allclose(dino_model.student.weight, initial_s_weight), \
           "Student weights did not update."
           
    # Teacher weights should have changed
    assert not torch.allclose(dino_model.teacher.weight, initial_t_weight), \
